...
public class ExceptionHandling {
	...
	public static void method() {
		...
		} finally { // 예외 발생 여부와 관계 없이 실행되는 영역, 선택 구문 (생략 가능)
	...
	public static void method2() throws FileNotFoundException { // 예외를 호출한 곳으로 전파
		...
		//switch (5) {
		case 0 :
			int x = 10 / 0;// ArimethicException (RuntimeException 상속) 예외가 발생하면 호출한 곳으로 보고
			break;
		case 1 :
			Object o = 10; //Integer
			String s = (String)o; //ClassCastException (RuntimeException 상속)
			break;
		case 2 :
			RuntimeException ex = new RuntimeException("사용자 정의 예외");
			throw ex;//강제로 예외 발생	
		case 3 :
			// checked exception : 반드시 예외처리 필요 ( 또는 throws 구문으로 예외 전파 또는 회피 )
